function [MTM] = amtm(sh,nw,k,dt,npad,sl,niter)
% function [MTM] = mtm(sh,nw,k,dt,npad,sl,niter)
%
% Function to determine the adaptative MTM power spectrum.
%
% Input:
%
%      sh       Time serie 
...
% condicion de partida
S0=psimat(:,1); % Primer estimado espectral
s2=sum(S0);
% s2=var(sty)
for j=1:5
   ...
   % s2=sum(S0)
    ...
    dk=[];
    for i=1:k
        dk(:,i)=(sqrt(lam(i)).*S0)./(lam(i)*S0+s2*(1-lam(i)));
    end
    
    S=sum(psimat.*dk.^2, 2)./sum(dk.^2, 2);
end
...
%figure; pcolor(P(1:500, :)); shading flat
%plot(P(:,1))

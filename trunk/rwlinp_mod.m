...
elseif nargin==1; % expects this to be the input filename or patH+filename, but
    ...
    if isempty(path1);
        ...
        if ~isdir(path1)
            error(['Could not find director ' path1]);
        end
    end
...
for n=1:nsers;
    ...
    % Loop over years
    for m = 1:nrows;
        c1=C(rowstart+m-1,:); % string data for this row
	if m~=nrows; 
	    c2=C(rowstart+m,:); 
	    nmtmp2 = c2(1:8);
	    if nmtmp2(8)=='-';
	    ...
	    yrgo2=str2double(c2(yrslot2:12));
	end
        if debug_level>=3
            disp(c1); % debug
        end
        ...
        if m==1; nms{n}=nmtmp1; end;
        yrgo1=str2double(c1(yrslot1:12));
        
	if m~=nrows; 
	    ngo=yrgo2-yrgo1;
	else;
	    if yrgo1>=0;  % If 0 or A.D.
                ngo=10-rem(yrgo1,10);
            else
                ngo=-rem(yrgo1,10);
	...
	for j=1:ngo;
		...
		val=c1(jgo:jsp);
	        ...
	        if all(isspace(val)) || str2double(val)==999 || str2double(val)==-9999 || j>10;
		    if m~=nrows; i=i+1; else; break; end
                else;
	            X(i)=str2double(c1(jgo:jsp));
                    i=i+1;
                end
	    elseif m~=nrows;
	        i=i+1;
	...
	if m==nrows;
	    decsp = str2double(c1(yrslot1:12));   % decade
	        ...
	        jgo=J1(j,1);
                jsp=J1(j,2);
                if all(isspace(c1(jgo:jsp))) || str2double(c1(jgo:jsp))==999 ||  str2double(c1(jgo:jsp))==-9999 || j>=10;
                    k2=0;
                else
	...
	if m==1; syr=yrgo1; end;
	if m==nrows; eyr=yrsp; end;
    end; % of for m=
    yrs(n,:)=[syr eyr I(n)];

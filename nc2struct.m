...
for i =1:length(varnames);
   ...
   if isfield(ncstruct.(varnames{i}),'attributes')
       if isfield(ncstruct.(varnames{i}).attributes,'missingvalue');
           missingVal=ncstruct.(varnames{i}).attributes.missingvalue;
           ncstruct.(varnames{i}).data(ncstruct.(varnames{i}).data==missingVal)=nan;
       end
       if isfield(ncstruct.(varnames{i}).attributes,'missing_value');
           missingVal=ncstruct.(varnames{i}).attributes.missing_value;
           ncstruct.(varnames{i}).data(ncstruct.(varnames{i}).data==missingVal)=nan;
       end

       if isfield(ncstruct.(varnames{i}).attributes,'FillValue');
           missingVal=ncstruct.(varnames{i}).attributes.FillValue;
           ncstruct.(varnames{i}).data(ncstruct.(varnames{i}).data==missingVal)=nan;
       end
       
       if isfield(ncstruct.(varnames{i}).attributes,'scalefactor')
           ncstruct.(varnames{i}).data=ncstruct.(varnames{i}).data.*ncstruct.(varnames{i}).attributes.scalefactor;
       end
   end
end
